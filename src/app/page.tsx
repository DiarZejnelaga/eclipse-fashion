...
import Link from 'next/link'; // Link import is essential for navigation
import { useEffect, useState, useRef } from 'react';
import { useSearchParams, useRouter } from 'next/navigation'; // For ?scrollTo=
...
// --- YOUR CountdownTimer COMPONENT CODE ---
...
// --- YOUR DealsImageRowSlider COMPONENT CODE ---
const DealsImageRowSlider = () => {
  // All items in this slider will link to the SAME "Deals of the Month" product page (ID 1001)
  // The 'id' here is just for unique key prop for React, the href will be the same.
  const allDealItems = [
  ...
  const renderItem = (itemData: { itemKey: string; src: string; alt: string; saleLabel: string; }, displayIdx: number) => {
    ...
    return (
      <Link key={uniqueKey} href="/product/1001"> {/* ALL SLIDER ITEMS LINK TO PRODUCT 1001 */}
...
// --- END OF DealsImageRowSlider ---

// --- YOUR Peaky Blinders Pointer Data & Logic ---
...
const getInstagramItemConfig = (num: number): InstagramItemConfig => { if (num === 1 || num === 7) {  return { aspect: 'aspect-[320/302]', isTall: false, applyShift: true }; } else if ([2, 4, 6].includes(num)) {  return { aspect: 'aspect-[256/380]', isTall: true, applyShift: false }; } else {  return { aspect: 'aspect-[256/302]', isTall: false, applyShift: true }; }};
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (sectionIdToScroll) {
      console.log(`[Homepage] Detected 'scrollTo' query parameter: ${sectionIdToScroll}`);
      const attemptScrollWithDelay = () => {
        const scrolled = scrollToElementById(sectionIdToScroll);
        if (scrolled) {
      ...
      <section id="deals-of-month-section" className="w-full py-12 md:py-16 lg:py-20 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-x-8 lg:gap-x-12 xl:gap-x-16 gap-y-10 items-center">
          <div className="space-y-5 md:space-y-6 text-center md:text-left">
            ...
            {/* Main "Deals" button links to Product ID 1001 */}
            <Link
              href="/product/1001" 
          ...
          <div className="mt-6 md:mt-0">
            {/* Slider items also link to Product ID 1001 */}
            <DealsImageRowSlider /> 
          </div>
        </div>
      </section>
      ...
      <div id="peaky-blinders-section" className="relative w-full h-[500px] sm:h-[550px] md:h-[600px] lg:h-[650px] overflow-hidden bg-black">
        ...
        <div className="relative z-10 flex h-full max-w-7xl mx-auto">
          ...
          <div className="w-2/5 flex flex-col justify-center pl-4 pr-6 sm:pl-6 sm:pr-8 md:pr-10 lg:pr-12 xl:pr-16 text-black">
            ...
            <div className="mb-4 sm:mb-5"><span className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800">99.99€</span></div> {/* Price for Peaky Blinders */}
            {/* "Buy Now" button in "Peaky Blinders" links to Product ID 2001 */}
            <Link
              href="/product/2001" 
